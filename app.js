...
const mongodb_URI =
  "mongodb+srv://summer:" +
  process.env.MONGO +
  "@cluster0.czfkc.mongodb.net/test";
...
app.post("/charities", async (req, res, next) => {
  const state = req.body.state;
  const url =
    "https://api.data.charitynavigator.org/v2/Organizations?app_id=d5c6fc87&app_key=" +
    process.env.CHARITY_API_KEY +
    "&state=" +
    state;
  try {
...
app.post("/charitiesRecord/add", isLoggedIn, async (req, res, next) => {
  try {
...
app.post(
  "/charitiesRecord/add/:charityName",
  isLoggedIn,
  async (req, res, next) => {
    try {
      const { name } = res.body; // get title and description from the body
      const userId = res.locals.user._id; // get the user's id
      let data = { name, userId }; // create the data object
      let item = new InterestCharity(data); // create the database object (and test the types are correct)
      await item.save(); // save the todo item in the database
      res.redirect("/charitiesRecord"); // go back to the todo page
    } catch (e) {
...
app.get(
  "/charitiesRecord/completed/:value/:itemId",
  isLoggedIn,
  async (req, res, next) => {
    try {
      const itemId = req.params.itemId; // get the id of the item to delete
